...
class ActionCatBoostTreeThompsonModel:
    """CatBoost-tree Thompson model using a single CatBoost tree (iterations=1)."""
    ...
    def fit(self, X, y):
        ...
        try:
            ...
            raise RuntimeError("catboost is required for ActionCatBoostTreeThompsonModel") from exc
    ...
    def _leaf_ids(self, X):
        ...
        self._ensure_fitted()
        X = self._to_2d(X)
        leaf_indexes = np.asarray(self.tree.calc_leaf_indexes(X))
        if leaf_indexes.ndim == 2:
            return leaf_indexes[:, 0].astype(int)
        return leaf_indexes.reshape(-1).astype(int)
...
class CatBoostTreeThompsonSamplingPolicy(TreeThompsonSamplingPolicy):
    """Tree Thompson policy that uses a single-tree CatBoost model per action."""

    def _build_model(self) -> ActionCatBoostTreeThompsonModel:
        return ActionCatBoostTreeThompsonModel(
            max_depth=self.max_depth,
            min_samples_leaf=self.min_samples_leaf,
            alpha0=self.alpha0,
            beta0=self.beta0,
...
class CatBoostTreeThompsonSamplingPolicyUpdateV1(CatBoostTreeThompsonSamplingPolicy):
    """Incremental CatBoost-tree updates via ActionCatBoostTreeThompsonModel.update_batch."""


class CatBoostTreeThompsonSamplingPolicyDummyRefit(CatBoostTreeThompsonSamplingPolicy):
    """CatBoost-tree variant that refits on full action history after each update."""
