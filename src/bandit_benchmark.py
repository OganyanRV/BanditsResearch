...
class BasePolicy:
    ...
    def action_probabilities(self, candidates: list[Action], features: list[float], row: dict[str, object]) -> dict[int, float]:
        if not candidates:
            return {}
        chosen = int(self.select(candidates, features, row))
        probs = {int(a): 0.0 for a in candidates}
        if chosen in probs:
            probs[chosen] = 1.0
        return probs

    def stochastic_select(self, candidates: list[Action], features: list[float], row: dict[str, object]) -> Action:
        if not candidates:
            raise ValueError("Empty candidate set")
        probs = self.action_probabilities(candidates, features, row)
        if not probs:
            return int(candidates[0])
        total = sum(max(0.0, float(probs.get(int(a), 0.0))) for a in candidates)
        if total <= 0:
            return int(candidates[0])
        r = random.random() * total
        csum = 0.0
        for a in candidates:
            csum += max(0.0, float(probs.get(int(a), 0.0)))
            if r <= csum:
                return int(a)
...
class RandomPolicy(BasePolicy):
    ...
    def action_probabilities(self, candidates: list[Action], features: list[float], row: dict[str, object]) -> dict[int, float]:
        ...
        return {int(a): p for a in candidates}
...
class EpsilonGreedyPolicy(BasePolicy):
    ...
    def action_probabilities(self, candidates: list[Action], features: list[float], row: dict[str, object]) -> dict[int, float]:
        ...
        n = len(candidates)
        base = self.epsilon / n
        probs = {int(a): base for a in candidates}
        best = max(candidates, key=lambda a: self.values.get(a, 0.0))
        probs[int(best)] += (1.0 - self.epsilon)
        return probs
...
class ThompsonSamplingPolicy(BasePolicy):
    ...
    def action_probabilities(self, candidates: list[Action], features: list[float], row: dict[str, object]) -> dict[int, float]:
        ...
        for a_raw in candidates:
            a = int(a_raw)
            sampled_scores[a] = float(
                self.rng.betavariate(
                    self.alpha.get(a, self.alpha0),
                    self.beta.get(a, self.beta0),
                )
            )

        total = float(sum(sampled_scores.values()))
        if total <= 0.0:
            p = 1.0 / len(candidates)
            return {int(a): p for a in candidates}

        return {int(a): float(sampled_scores[int(a)] / total) for a in candidates}
...
class LaplaceThompsonViaBayesianLogRegPolicy(BasePolicy):
    ...
    def action_probabilities(self, candidates: list[Action], features: list[float], row: dict[str, object]) -> dict[int, float]:
        ...
        x = np.asarray(features, dtype=np.float64).reshape(1, -1)
        n_mc = 128
        counts = {int(a): 0 for a in candidates}

        saved_states: dict[int, dict[str, object]] = {}
        for a_raw in candidates:
            a = int(a_raw)
            model = self._get_model(a)
            saved_states[a] = model.rng.bit_generator.state
        ...
        try:
            ...
            for a_raw in candidates:
                a = int(a_raw)
                self._get_model(a).rng.bit_generator.state = saved_states[a]

        return {int(a): float(counts[int(a)] / n_mc) for a in candidates}
...
class NeuralLaplaceThompsonViaBayesianLogRegPolicy(BasePolicy):
    ...
    def action_probabilities(self, candidates: list[Action], features: list[float], row: dict[str, object]) -> dict[int, float]:
        return self._base.action_probabilities(candidates, self._transform_features(features), row)
...
def evaluate_policy(
    ...
    online_update: bool,
    update_frequency: Literal["daily", "step_2p5"] = "daily",
    evaluation_mode: Literal["replay", "stochastic_ips"] = "replay",
    ...
    snips_sensitive_weight_sum = 0.0
    sensitive_rows = 0
    sensitive_used = 0
    sensitive_total_reward = 0.0
    used = 0
    replay_matches = 0
    ...
    for batch_start in range(0, len(test_rows), batch_size):
        ...
        for offset, row in enumerate(rows_batch):
            ...
            deterministic_action = int(actions_batch[offset])
            eval_probs = policy.action_probabilities(candidates, features, row)
            action = int(policy.stochastic_select(candidates, features, row)) if evaluation_mode == "stochastic_ips" else deterministic_action
            ...
            logged_match = int(action == logged_action)
            propensity = float(row.get("propensity", 0.0) or 0.0)

            if evaluation_mode == "stochastic_ips":
                pi_logged = float(eval_probs.get(logged_action, 0.0))
                ips_weight = (pi_logged / propensity) if propensity > 0 else 0.0
            else:
            ...
            if len(candidates) > 1:
                ...
                snips_sensitive_weight_sum += ips_weight
                sensitive_seen += 1
    ...
    snips_ctr_sensitive = (ips_sensitive_reward_sum / snips_sensitive_weight_sum) if snips_sensitive_weight_sum > 0 else 0.0
    sensitive_ctr = (sensitive_total_reward / sensitive_used) if sensitive_used else 0.0
    ...
    metrics_df = pd.DataFrame([
        {
            ...
            "snips_ctr_sensitive": snips_ctr_sensitive,
            "sensitive_ctr": sensitive_ctr,
